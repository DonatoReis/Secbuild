...
init_install() {
  ...
  if [[ ! -f $HOME/.local/.arno_init_install_successful ]]; then
    apt -y install python3-pip apt-transport-https curl libcurl4-openssl-dev libssl-dev xvfb debian-keyring sn0int bison fail2ban firebird-dev flex grc git jq libgcrypt20-dev libgpg-error-dev libgtk2.0-dev libidn11-dev libmemcached-dev libmysqlclient-dev libpcap-dev libpcre3-dev libpq-dev libssh-dev libssl-dev libsvn-dev net-tools p7zip rubygems zsh ufw zsh-autosuggestions zsh-syntax-highlighting virtualbox-guest-x11 jq ruby-full libcurl4-openssl-dev ruby virtualbox-guest-utils libxml2 libxml2-dev libxslt1-dev ruby-dev build-essential libgmp-dev hcxtools hcxdumptool zlib1g-dev perl chromium zsh fonts-powerline libio-socket-ssl-perl libdbd-sqlite3-perl libclass-dbi-perl libio-all-lwp-perl libparallel-forkmanager-perl libredis-perl libalgorithm-combinatorics-perl gem git cvs subversion git bzr mercurial build-essential libssl-dev libffi-dev python2-dev python2 python-dev-is-python3 ruby-ffi-yajl python-setuptools libldns-dev nmap rename docker.io parsero apache2 amass joomscan uniscan ssh tor privoxy wifite proxychains4 hashcat aptitude synaptic lolcat python3.9-venv dialog golang-go exploitdb exploitdb-papers exploitdb-bin-sploits graphviz kali-desktop-gnome virtualenv reaver bats metagoofil openssl feroxbuster
    sudo $SUDO_OPT pip3 install --upgrade pip
